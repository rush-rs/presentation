\input{preamble/misc}
\input{preamble/tabularx}
\input{preamble/drawstack}

\input{|"lirstings include-tex"}
% Set `deps/rush` as path prefix in all lirstings, is ignored for files under other directories
\NewCommandCopy{\oldLirsting}{\Lirsting}
\renewcommand{\Lirsting}[2][]{\oldLirsting[path prefix={deps/rush},#1]{#2}}

\input{preamble/tikz}

\title{Die Umwandlung von Quelltext in Maschinensprache}
\date{17. Mai 2023}
\author{Silas Groh, Mik Müller}
\institute{Carl-Fuhlrott-Gymnasium}

\titlegraphic{\rushlogo{y=0.80pt, x=0.80pt, yscale=-0.17, xscale=0.17}}

\begin{document}
\begin{frame}
	\titlepage
\end{frame}

\input{content/einstieg}
\input{content/lexikalisch_syntaktisch}
\input{content/semantische_analyse}
\input{content/interpreter}
\input{content/vm}
\input{content/high_level}
\input{content/wasm}
\input{content/llvm}
\input{content/low_level}
\input{content/riscv}
\input{content/x64}
\input{content/low_level_conclusions}
\input{content/anmerkungen}

% \listoffigures
% \listoftables
% \listof{listing}{Programmblockverzeichnis}
% \nocite{*}

\begin{frame}[shrink=30]
	\frametitle{Quellenverzeichnis}
    \vspace{1cm}
    \small\printbibliography[heading=bibintoc]
\end{frame}
\end{document}
